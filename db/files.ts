...
import { apiEndpoint } from "@/app/auth/auth"
...
// Convert File to array of integers
const fileToIntArray = async (file: File): Promise<number[]> => {
...
export const createFile = async (
  ...
  const intArray = await fileToIntArray(file)

  // Add the username
  const response = await fetch(`${apiEndpoint}/index/create`, {
    method: "POST",
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ 
      email: createdFile.user_id,
      data: intArray 
    })
  })
